<?php
/**
 * Cast Member Management Page (Admin)
 * Lists all cast members with edit and view options.
 */
...
if (isset($_GET['delete'])) {
    $cast_id = (int) $_GET['delete'];
    
    // Check if cast member exists
    $check_sql = "SELECT name FROM cast_members WHERE cast_id = $cast_id";
    $check_result = myQuery($check_sql);
    if (mysqli_num_rows($check_result) > 0) {
        $cast_data = mysqli_fetch_assoc($check_result);
        $cast_name = $cast_data['name'];
        
        // Delete cast member (CASCADE will handle movie_cast relationships)
        $delete_sql = "DELETE FROM cast_members WHERE cast_id = $cast_id";
        if (myQuery($delete_sql)) {
            // Log admin action
            $admin_id = $_SESSION['user_id'];
            $log_sql = "INSERT INTO admin_logs (admin_id, action_type, target_type, target_id, description) 
                        VALUES ($admin_id, 'cast_delete', 'cast', $cast_id, 'Deleted cast member: $cast_name')";
            myQuery($log_sql);
...
// Retrieve cast members with movie count, search, and pagination
$cast_sql = "SELECT c.*, 
             (SELECT COUNT(*) FROM movie_cast WHERE cast_id = c.cast_id) as movie_count
             FROM cast_members c 
             $where_clause
             ORDER BY c.name ASC
...
$count_sql = "SELECT COUNT(*) as total FROM cast_members c $where_clause";
$count_result = myQuery($count_sql);
$total_cast = mysqli_fetch_assoc($count_result)['total'];
$total_pages = ceil($total_cast / $per_page);
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>Manage Cast Members - xGrab</title>
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mb-6 fade-in">
            <form method="get" action="" class="flex gap-3">
                <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>"
                    placeholder="Search cast members by name or biography..."
        ...
        <?php if (!empty($search)): ?>
            <div class="mb-4 text-gray-400 fade-in">
                Found <?php echo $total_cast; ?> cast member(s) matching "<?php echo htmlspecialchars($search); ?>"
            </div>
        <?php endif; ?>
        ...
        <?php if (mysqli_num_rows($cast_result) > 0): ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8 fade-in">
                <?php while ($cast = mysqli_fetch_assoc($cast_result)): ?>
                    ...
                    <div class="bg-gray-800 rounded-xl shadow-lg border border-gray-700 overflow-hidden hover:shadow-xl hover:border-red-500 transition-all duration-300 transform hover:-translate-y-1">
                        ...
                        <div class="relative h-64 bg-gray-700">
                            ...
                            <?php endif; ?>
                            <!-- Movie Count Badge -->
                            <!-- Display number of movies this cast member appears in -->
                            <div class="absolute top-3 right-3 bg-red-600 text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg">
                                <?php echo $cast['movie_count']; ?> Movie<?php echo $cast['movie_count'] != 1 ? 's' : ''; ?>
                            </div>
                        </div>
                        ...
                        <div class="p-5">
                            ...
                            <?php if (!empty($cast['biography'])): ?>
                                <!-- Biography Preview -->
                                <!-- Display first 120 characters of biography with ellipsis if longer -->
                                <p class="text-sm text-gray-400 mb-4 line-clamp-3">
                                    <?php echo htmlspecialchars(substr($cast['biography'], 0, 120)); ?>
                                    <?php echo strlen($cast['biography']) > 120 ? '...' : ''; ?>
                                </p>
                            <?php else: ?>
                            ...
                            <!-- Action Buttons -->
                            <!-- Edit, View, and Delete actions for this cast member -->
                            <div class="flex gap-2 pt-3 border-t border-gray-700">
                                <a href="edit.php?id=<?php echo $cast['cast_id']; ?>"
                                ...
                                <a href="../../cast/details.php?id=<?php echo $cast['cast_id']; ?>"
                                    class="flex-1 px-3 py-2 bg-gray-700 text-white rounded-lg hover:bg-gray-600 transition-all duration-300 text-center text-sm font-medium">
                                    View
                                </a>
                                <a href="?delete=<?php echo $cast['cast_id']; ?><?php echo !empty($search) ? '&search=' . urlencode($search) : ''; ?>"
                                    class="px-3 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-all duration-300 text-center text-sm font-medium"
                                    onclick="return confirm('Are you sure you want to delete <?php echo htmlspecialchars(addslashes($cast['name'])); ?>? This will also remove them from all movies.');">
                                    Delete
                                </a>
                            </div>
                        </div>
        ...
        <?php else: ?>
            <div class="bg-gray-800 rounded-xl shadow-md p-12 text-center border border-gray-700 fade-in">
                ...
                <p class="text-gray-400 text-lg mb-2">
                    <?php echo !empty($search) ? 'No cast members found matching your search.' : 'No cast members found.'; ?>
                </p>
                <?php if (!empty($search)): ?>
                    <a href="index.php"
