<?php
/**
 * Edit Cast Member Page (Admin)
 * Allows administrators to update existing cast member information.
 * Handles cast member data updates and photo upload.
 */
...
// Retrieve current cast member information for form pre-population
...
// Process cast member update form submission
if (isset($_POST['submit'])) {
    ...
    $old_photo_url = $cast_member['photo_url'];
    $new_photo_uploaded = false;

    // Handle cast member photo upload (optional - keep existing if not uploaded)
    $photo_url = $cast_member['photo_url']; // Keep existing photo by default
    ...
    if (empty($name)) {
        ...
        // Update cast member record with new information
        $photo_value = $photo_url ? "'$photo_url'" : 'NULL';
        $update_sql = "UPDATE cast_members SET 
                       name = '$name',
        ...
        if (myQuery($update_sql)) {
            ...
            $log_sql = "INSERT INTO admin_logs (admin_id, action_type, target_type, target_id, description) 
                        VALUES ($admin_id, 'cast_update', 'cast', $cast_id, 'Updated cast member: $name')";
            myQuery($log_sql);

            $success = "Cast member updated successfully!";
            // Reload cast member data to reflect changes
            $cast_result = myQuery($cast_sql);
            $cast_member = mysqli_fetch_assoc($cast_result);
        } else {
...
<head>
    ...
    <title>Edit Cast Member - xGrab</title>
        ...
        <div class="bg-gray-800 rounded-xl shadow-lg p-6 md:p-8 max-w-4xl border border-gray-700 fade-in">
            ...
            <form method="post" enctype="multipart/form-data" class="space-y-6">
                <div>
                    ...
                    <input type="text" name="name" value="<?php echo htmlspecialchars($cast_member['name']); ?>" required
                ...
                <div>
                    ...
                    <?php if ($cast_member['photo_url']): ?>
                        ...
                        <div id="currentPhoto" class="mb-3">
                            ...
                            <img src="<?php echo htmlspecialchars(getImagePath($cast_member['photo_url'], 'cast')); ?>?v=<?php echo time(); ?>"
                ...
                <div>
                    ...
                    <textarea name="biography" rows="6" placeholder="Enter cast member biography"
                        class="w-full px-4 py-3 bg-gray-700 border-2 border-gray-600 rounded-lg focus:border-red-500 focus:ring-2 focus:ring-red-500 transition-all duration-300 text-gray-100 placeholder-gray-400"><?php echo htmlspecialchars($cast_member['biography'] ?? ''); ?></textarea>
                </div>
                ...
                <div class="flex space-x-3 pt-2">
                    ...
                    <a href="../../cast/details.php?id=<?php echo $cast_id; ?>"
                        class="px-6 py-3 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1 inline-block font-medium">
                        View Cast Member
                    </a>
                </div>
